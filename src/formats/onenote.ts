...
export class OneNoteImporter extends FormatImporter {
  ...
  init() {
    ...
    new Setting(this.modal.contentEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(false).onChange((value) => (this.importIncompatibleAttachments = value))
      );

    // TODO: Add a setting for importDrawingsOnly when InkML support is complete

    this.microsoftAccountSetting = new Setting(this.modal.contentEl)
      .setName('Sign in with your Microsoft account')
      .setDesc('You need to sign in to import your OneNote data.')
      .addButton((button) =>
        button.setCta().setButtonText('Sign in').onClick(() => {
          ...
          window.open(
            `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?${requestBody.toString()}`
          );
        })
      );
  ...
  async authenticateUser(protocolData: ObsidianProtocolData) {
    try {
      ...
      if (!tokenResponse.access_token) {
        throw new Error(
          `Unexpected data was returned instead of an access token. Error details: ${tokenResponse}`
        );
      }

      this.graphData.accessToken = tokenResponse.access_token;

      const userData: User = await this.fetchResource(
      ...
      this.modal.contentEl
        .createEl('div', { text: 'An error occurred while trying to sign you in.' })
  ...
  async showSectionPickerUI() {
    ...
    for (const notebook of notebooks) {
      ...
      new Setting(notebookDiv)
        ...
        .setDesc(
          `Last edited on: ${(moment.utc(notebook.createdDateTime)).format('Do MMMM YYYY')}. Contains ${notebook.sections?.length} sections.`
        )
        .addButton((button) =>
          button
            .setCta()
            ...
            .onClick(() => {
              ...
              this.selectedSections.push(
                ...(notebook.sectionGroups || []).flatMap((element) => element?.sections || [])
              );
            })
        );
  ...
  async import(progress: ImportContext): Promise<void> {
    ...
    for (let section of this.selectedSections) {
      ...
      if (section.parentSectionGroup) {
        let sectionGroupFolder: TFolder = await this.createFolders(
          outputFolder.path + '/' + section.parentSectionGroup.displayName
        );
        sectionFolder = await this.createFolders(
          sectionGroupFolder.path + '/' + section.displayName
        );
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        try {
          ...
          this.processFile(progress, sectionFolder, outputFolder, await this.fetchResource(
  ...
  getAllAttachments(pageHTML: string): HTMLElement {
    ...
    for (const video of videos) {
      ...
      if (video.src.contains('youtube.com') || video.src.contains('youtu.be')) {
        const embedNode = document.createTextNode(
          `![Embedded YouTube video](${video.src})`
        );
  ...
  async fetchAttachmentQueue(
    ...
    if (this.attachmentQueue.length >= 1) {
      ...
      if (this.useDefaultAttachmentFolder)
        attachmentPath = await this.app.vault.getAvailablePathForAttachments(
